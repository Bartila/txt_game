...
if (isset($_GET['buy'])) {
    ...
    if ($buy) {
        ...
        try{
            $stmt = $pdo->prepare("UPDATE characters SET health = ? WHERE id = ?");
            $stmt->execute([$_SESSION['character']['health'],$_SESSION['char_id']]);
...
<body>
    ...
    <div class="main-window">
        ...
        <?php echo '<p> Health: ' . $_SESSION['character']['health'] . '/' . $_SESSION['character']['maxhealth'] . '</br> Coins: ' . $_SESSION['character']['coins'] . '</p>'; ?>
        <div class="options">
            <?php
